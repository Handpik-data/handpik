import os
import re
import json
import asyncio
import aiohttp
import requests
from bs4 import BeautifulSoup
from typing import List, Dict
import requests
from requests.adapters import HTTPAdapter
from urllib3 import Retry
from interfaces.base_scraper import BaseScraper
from datetime import datetime
from utils.LoggerConstants import RANGJA_LOGGER
from urllib.parse import urljoin, urlparse
import time


class RangjaScraper(BaseScraper):
    def __init__(self, proxies=None, request_delay=1):
        super().__init__(
            base_url="https://myrangja.com/",
            logger_name=RANGJA_LOGGER,
            proxies=proxies,
            request_delay=request_delay
        )
        self.module_dir = os.path.dirname(os.path.abspath(__file__))
        self.store_name = "rangja"
        self.all_product_links_ = []

    async def get_unique_urls_from_file(self, filename):
        if not isinstance(filename, str) or not filename.strip():
            raise ValueError("Filename must be a non-empty string.")
        
        filepath = os.path.join(self.module_dir, filename)
        if not os.path.exists(filepath):
            raise FileNotFoundError(f"The file '{filename}' does not exist.")
        
        with open(filepath, 'r') as file:
            return list(set(line.strip() for line in file if line.strip()))


    async def scrape_pdp(self, product_link):
        if product_link in self.all_product_links_:
            return None

        self.all_product_links_.append(product_link)
        product_data = {
            'store_name': self.store_name,
            'title': None,
            'sku': None,
            'description': None,
            'currency': None,
            'original_price': None,
            'sale_price': None,
            'images': [],
            'brand': None,
            'availability': None,
            'category': None,
            'product_url': product_link,
            'variants': [],
            'attributes': {},
            'raw_data': {}
        }

        try:
            response = await self.async_make_request(product_link)
            soup = BeautifulSoup(response.text, "html.parser")

            try:
                product_title_tag = soup.find('h1', class_='productView-title')
                if product_title_tag:
                    span_tag = product_title_tag.find('span')
                    if span_tag and span_tag.get_text(strip=True):
                        product_data["title"] = span_tag.get_text(strip=True)
                    else:
                        product_data["title"] = product_title_tag.get_text(strip=True)
            except Exception as e:
                self.log_debug(f"Exception occurred while scraping product's title: {e}")

            try:
                breadcrumbs = []
                breadcrumb_container = soup.find('nav', class_='breadcrumb breadcrumb-left')
                if breadcrumb_container:
                    a_tags = breadcrumb_container.find_all('a')
                    for a_tag in a_tags:
                        crumb = a_tag.get_text(strip=True)
                        breadcrumbs.append(crumb)
                    span_tags = breadcrumb_container.find_all('span')
                    for span_tag in span_tags:
                        text = span_tag.get_text(strip=True)
                        if text and not span_tag.find('svg'):
                            breadcrumbs.append(text)
                product_data['attributes']['breadcrumbs'] = breadcrumbs
            except Exception as e:
                self.log_debug(f"Exception occurred while scraping breadcrumbs: {e}")

            try:
                sku_container = soup.find('div', class_='productView-info-item')
                if sku_container:
                    name_span = sku_container.find('span', class_='productView-info-name')
                    value_span = sku_container.find('span', class_='productView-info-value')
                    if name_span and 'SKU:' in name_span.get_text(strip=True) and value_span:
                        product_data['sku'] = value_span.get_text(strip=True)
            except Exception as e:
                self.log_debug(f"Exception occurred while scraping SKU: {e}")

            try:
                def extract_price(text):
                    text = text.replace("From", "").strip()
                    currency_match = re.match(r'^(\D+)', text)
                    currency = currency_match.group(1).strip() if currency_match else None
                    price = re.sub(r'[^\d.]', '', text).lstrip('.')
                    return price, currency

                price_container = soup.find('div', class_='productView-price')
                if price_container:
                    price_div = price_container.find('div', class_='price')
                    if price_div:
                        sale_div = price_div.find('div', class_='price__sale')
                        if sale_div:
                            original_s_tag = sale_div.find('s', class_='price-item--regular')
                            original_money = original_s_tag.find('span', class_='money') if original_s_tag else None
                            sale_span_tag = sale_div.find('span', class_='price-item--sale')
                            sale_money = sale_span_tag.find('span', class_='money') if sale_span_tag else None
                            if original_money and sale_money:
                                original, currency = extract_price(original_money.get_text(strip=True))
                                sale, _ = extract_price(sale_money.get_text(strip=True))
                                product_data['original_price'] = original
                                product_data['sale_price'] = sale
                                product_data['currency'] = currency
                        else:
                            regular_div = price_div.find('div', class_='price__regular')
                            if regular_div:
                                regular_money_tag = regular_div.find('span', class_='money')
                                if regular_money_tag:
                                    price, currency = extract_price(regular_money_tag.get_text(strip=True))
                                    product_data['original_price'] = price
                                    product_data['currency'] = currency
            except Exception as e:
                self.log_debug(f"Error extracting price data: {e}")

            try:
                breadcrumb_component = soup.find('breadcrumb-component', class_='breadcrumb-container')
                if breadcrumb_component:
                    nav_tag = breadcrumb_component.find('nav', class_='breadcrumb')
                    categories = [
                        tag.get_text(strip=True)
                        for tag in nav_tag.find_all(['a', 'span'])
                        if tag.get_text(strip=True) and tag.get_text(strip=True).lower() != 'home'
                    ]
                    product_data["category"] = categories
            except Exception as e:
                self.log_debug(f"Exception occurred while extracting categories from breadcrumbs: {e}")

            try:
                variants = []
                size_fieldset = soup.find('fieldset', {'data-product-attribute': 'set-rectangle'})
                if size_fieldset:
                    labels = size_fieldset.find_all('label', class_='product-form__label')
                    for label in labels:
                        size = label.find('span', class_='text').text.strip()
                        is_available = 'available' in label.get('class', [])
                        variants.append({
                            'size': size,
                            'availability': 'true' if is_available else 'false'
                        })
                product_data['variants'] = variants
            except Exception as e:
                self.log_debug(f"Exception occurred while scraping sizes: {e}")

            try:
                product_images = []
                img_tags = soup.find_all('img', id=lambda x: x and x.startswith('product-featured-image'))
                for img_tag in img_tags:
                    srcset = img_tag.get('srcset')
                    if srcset:
                        last_entry = srcset.split(',')[-1].split(' ')[0].strip()
                        if last_entry.startswith('//'):
                            last_entry = 'https:' + last_entry
                        elif last_entry.startswith('/'):
                            last_entry = 'https://myrangja.com' + last_entry
                        product_images.append(last_entry)
                    else:
                        src = img_tag.get('src')
                        if src:
                            if src.startswith('//'):
                                src = 'https:' + src
                            elif src.startswith('/'):
                                src = 'https://myrangja.com' + src
                            product_images.append(src)
                seen = set()
                unique_images = []
                for img in product_images:
                    if img not in seen:
                        unique_images.append(img)
                        seen.add(img)
                product_data['images'] = unique_images
            except Exception as e:
                self.log_debug(f"Exception occurred while scraping product images: {e}")

            try:
                toggle_content = soup.find('div', class_='toggle-content', id='tab-shipping-amp-return-mobile')
                shipping_and_returns = ""
                if toggle_content:
                    paragraphs = toggle_content.find_all(['h3', 'p'])
                    content_lines = [tag.get_text(strip=True) for tag in paragraphs if tag.get_text(strip=True)]
                    shipping_and_returns = "\n".join(content_lines)
                product_data['raw_data']['shipping_and_return'] = shipping_and_returns
            except Exception as e:
                self.log_debug(f"Exception occurred while scraping shipping and returns: {e}")

            try:
                description_text = ""
                desc_div = soup.find('div', class_='toggle-content is-active show-mobile', id='tab-description-mobile')
                if desc_div:
                    p_tags = desc_div.find_all('p')
                    for p in p_tags:
                        text = p.get_text(separator=" ", strip=True)
                        if text:
                            description_text = text
                            break
                product_data['description'] = description_text
            except Exception as e:
                self.log_debug(f"Exception occurred while scraping description: {e}")

        except Exception as e:
            self.log_debug(f"Error scraping PDP {product_link}: {str(e)}")

        return product_data




    async def scrape_products_links(self, url):
        from urllib.parse import urljoin

        all_product_links = []
        current_url = url

        while True:
            try:
                self.log_info(f"Scraping: {current_url}")
                response = await self.async_make_request(current_url)
                soup = BeautifulSoup(response.text, 'html.parser')

                # Select <a> tags with class 'card-link'
                product_links = soup.select('a.card-link')

                if not product_links:
                    self.log_info("No product links found on this page. Stopping.")
                    break

                # Extract and add product URLs
                for link_tag in product_links:
                    href = link_tag.get('href')
                    if href:
                        product_url = urljoin(self.base_url, href)
                        if product_url not in all_product_links:
                            all_product_links.append(product_url)

                self.log_info(f"Collected {len(all_product_links)} product links so far.")

                # Pagination: look for next page link (adjust selector based on site)
                next_page_link = soup.select_one('a.next')
                if next_page_link:
                    next_href = next_page_link.get('href')
                    if next_href:
                        current_url = urljoin(self.base_url, next_href)
                        continue
                    else:
                        self.log_info("Next page href not found. Stopping pagination.")
                        break
                else:
                    self.log_info("No next page link found. Finished scraping all pages.")
                    break

            except Exception as e:
                self.log_error(f"Error scraping {current_url}: {e}")
                break

        self.log_info(f"Total collected {len(all_product_links)} product links.")
        return all_product_links


         
    async def scrape_category(self, url):
        all_products = []
        all_products_links = await self.scrape_products_links(url)
        for product_link in all_products_links:
            pdp_data = await self.scrape_pdp(product_link)
            if pdp_data is not None:
                all_products.append(pdp_data)
        
        return all_products

    async def scrape_data(self):
        final_data = []
        try:
            category_urls = await self.get_unique_urls_from_file(
                os.path.join(self.module_dir, "categories.txt")
            )
            for url in category_urls:
                products = await self.scrape_category(url)
                final_data.extend(products)
            if final_data:
                saved_path = await self.save_data(final_data)  # ✅ FIXED
                if saved_path:
                    self.log_info(f"Total {len(category_urls)} categories")
                    self.log_info(f"Saved {len(final_data)} products to {saved_path}")
                    self.log_info(f"Product Sample Data: {json.dumps(final_data[0], separators=(',', ':'))}")
            else:
                self.log_error("No data scraped")
        except Exception as e:
            self.log_error(f"Scraping failed: {str(e)}")
