...
class ImageScraper(BaseScraper):
    ...
    async def scrape_pdp(self, product_link):
        ...
        try:
            ...
            try:
                ...
                if price_wrapper:
                    original_tag = price_wrapper.select_one(
                    ...
                    if original_tag and sale_tag:
                        ...
                        original_price = (
                            original_price_match[0] if original_price_match else None
                        )
                        ...
                        sale_price_match = re.findall(r"[\d,]+", sale_price_str)
                        original_price = (
                            sale_price_match[0] if sale_price_match else None
                        )
                        sale_price = None
                    ...
                    if original_price:
                        try:
                            currency_match = re.match(
                                r"([^\d]+)", original_price_str.strip()
                            )
                            currency = (
                                currency_match.group(1).strip()
                                if currency_match
                                else None
                            )
                            ...
                            if sale_val and orig_val > sale_val:
                                discount = round(
                                    ((orig_val - sale_val) / orig_val) * 100
                                )
            ...
            try:
                label = soup.find(
                    "label", class_="product-form_custom_label Color--label"
                )
                if label:
                    texts = [
                        t for t in label.stripped_strings if "Variant sold out" not in t
                    ]
                    if texts:
                        texts[0]
            ...
            try:
                shipment_text = None
                shipment_div = soup.select_one(
                    "div#ProductAccordion-collapsible_tab_8LbGXL-template--24274139414891__main"
                )
                if shipment_div:
                    for br in shipment_div.find_all("br"):
                        br.replace_with("\n")
                    paragraphs = [
                        p.get_text(strip=True).replace("\xa0", " ")
                        for p in shipment_div.find_all("p")
                    ]
                    shipment_text = "\n\n".join(paragraphs)
                ...
                for tag in details_tags:
                    ...
                    if (
                        summary
                        and "CARE INSTRUCTIONS" in summary.get_text(strip=True).upper()
                    ):
                        p_tag = tag.find("p")
                        if p_tag:
                            for br in p_tag.find_all("br"):
                                br.replace_with("\n")
                            product_care_text = (
                                p_tag.get_text(strip=True).replace("\xa0", " ").strip()
                            )
                        break
                ...
                self.log_debug(
                    f"Exception occurred while scraping shipment info or product care: {e}"
                )
                product_data["raw_data"] = {"product_care": None, "shipment_info": None}

            try:
                variants = []
                color_labels = soup.find_all(
                    "label", class_="product-form_custom_label Color--label"
                )
                colors = []
                for label in color_labels:
                    color_text = (
                        ...
                        .replace("Variant sold out or unavailable", "")
                        .strip()
                    )
                    if color_text:
                ...
                if size_fieldset and colors:
                    size_inputs = size_fieldset.find_all(
                        "input", {"name": "Size", "type": "radio"}
                    )
                    for input_tag in size_inputs:
                        size_value = input_tag.get("value", "").strip()
                        is_disabled = input_tag.has_attr(
                            "disabled"
                        ) or "disabled" in input_tag.get("class", [])
    ...
    async def scrape_products_links(self, url):
        try:
            ...
            while current_url:
                self.log_info(f"Scraping product links from: {current_url}")
                ...
                if next_page_tag and next_page_tag.get("href"):
                    ...
                    self.log_info("No more pages found. Stopping.")
                    break
        ...
        try:
            ...
            if final_data:
                ...
                if saved_path:
                    ...
                    self.log_info(
                        f"Product Sample Data: {json.dumps(final_data[0], separators=(',', ':'))}"
                    )
            else:
